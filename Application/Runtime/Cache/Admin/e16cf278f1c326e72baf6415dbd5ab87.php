...
<body>
	    ...
	    <ul class="forminfo">
	    <li><label>标题</label><input name="title" type="text" class="dfinput" value=''required='required'/><i>标题不能超过30个字符</i></li>
	    <li><label>文章分类</label>
	    	<select name='cat_id'style="width:345px; height:32px; line-height:32px; border-top:solid 1px #c3ab7d; border-left:solid 1px #c3ab7d; border-right:solid 1px #e7d5ba; border-bottom:solid 1px #e7d5ba; background: rgba(0, 0, 0, 0)  repeat-x scroll 0 0;">
				<?php if(is_array($catList)): $i = 0; $__LIST__ = $catList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["cat_id"]); ?>"><?php echo str_repeat('&nbsp;&nbsp;&nbsp;&nbsp;',$vo['lev']); echo ($vo["cat_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	    	</select>
	    </li>
	    <li><label>文章图片</label><input name="image" type="file" class="dfinput"  /><i></i></li>

		

	    <li>
			<label>内容</label>
			<textarea name="body" id="content" style="width:1000px; min-height:420px" class="textinput" >
