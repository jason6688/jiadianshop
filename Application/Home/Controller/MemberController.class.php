...
class MemberController extends Controller {
    public function index(){
		  ...
		  $payment_order=M("Order")->where(array('user_id'=>$user_id,'order_state'=>1))->select();
		  $payment_count=count($payment_order);
		  //查找已经收货的订单
		  $finish_order=M("Order")->where(array('user_id'=>$user_id,'order_state'=>3))->field('order_id')->select();
		  $finish_order_id=array();
		  foreach($finish_order as $finish){
		     $finish_order_id[]=$finish['order_id'];
		  }
		  //查询需要评价的商品总数目
		  //$comment_goods=M('Order_desc')->where(array('order_id'=>array('in',$finish_order_id)))->select();
