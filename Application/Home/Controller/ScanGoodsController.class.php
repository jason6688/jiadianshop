	...
	public  function index(){
		  ...
		  if(empty($cat_info)){
		     ...
		     if(strlen($cat_info)){
			  ...
			  $goods_info=M("Goods")->field('goods_id,goods_name,market_price,shop_price,goods_thumb_img')->where(array('goods_id'=>array('in',$goods_ids)))->select();
			   //循环遍历处理
              foreach($goods_info as $info){
			        //生成URL路径
				   $url=U('Product/index',array('g'=>$info['goods_id']));
				   //判断价格
				   if($info['market_price']>$info['shop_price']&&$info['shop_price']!=0){
				     ...
				     <li>
						   <div>
							<p  class="p-img">
							  ...
							  <img  src="{$info['goods_thumb_img']}"  alt="{$info['goods_name']}">
							  </a>
							</p>
							<p  class="p-name">
